...
export const getAllMyTransactions = async (req, res) => {
  try {
    ...
    if (req.query.search) filter.transaction_name = { $regex: req.query.search, $options: 'i' };
    const allTransactions = await Transaction.find(filter).sort({ createdAt: -1 }).populate('account_id');
    let typesSummary = {};
    let totalAmount = 0;
    let methodsSummary = {};
    allTransactions.forEach(tx => {
      ...
      if (!methodsSummary[method]) methodsSummary[method] = { count: 0, total: 0 };
    ...
    res.json({
      ...
      summary: { total_transactions: allTransactions.length, total_amount: totalAmount, by_type: typesSummary, by_method: methodsSummary }
    });
  } catch (error) {
    res.status(500).json({ success: false, message: 'Error al obtener todas las transacciones', error: error.message });
  }
};
